...
```
工作流程：
  用户本地运行 AI Agent（OpenClaw/Claude Code/Codex/任意工具）
    ↓
  用户已在 Agent 中配好自己的 API Key
    ↓
  安装 claw-miner → 选择共享哪些 LLM → 设置额度上限
    ↓
  网络路由推理请求到用户节点 → 用户的 Agent 工具发起 LLM 调用
    ↓
  用户赚取 CLAW
```

**这与所有 AI Agent 工具使用 LLM 的方式完全一致**：
- OpenClaw 用用户的 Key 调 LLM = 正规使用
- Claude Code 用用户的 Key 调 LLM = 正规使用
- claw-miner 用用户的 Key 调 LLM = 完全相同的模式
- 用户选择共享闲置额度是用户自己的决定
